"""
Cycle38小修小补(b)：偏度因子(第200条)的Fama-French三因子alpha检验——
第200条只报告了CAGR/Sharpe/MaxDD，这次检验超额收益是否在因子暴露之外
有真实alpha，还是只是又一次换皮的已知因子暴露(呼应第38条对价值因子
做过的同样检验)。用2022-2026代表性区间(数据质量最高、point-in-time
成分股修正过)。
"""
...
from run_skewness_factor_test import backtest_skew, SCRATCH
...
def main():
    ...
    print("跑偏度因子策略拿逐日净值...")
    daily_nav = backtest_skew(price_df, membership)
    ...
    print(f"\n偏度因子: 年化alpha={alpha_annualized*100:+.2f}%, alpha_t值={tvals[0]:.2f} "
