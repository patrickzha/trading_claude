"""
Cycle59后续：value2%/lowvol5%非对称止损组合(小修(d)的初步发现)三段区间点估计
...
for label, price_cache_path, mom_cache_path, is_pit in PERIODS:
    ...
    print(f"  value3%(现有推荐): Sharpe={r_base['Sharpe']:.3f}, MaxDD={r_base['MaxDD']*100:.2f}%")
    print(f"  value2%/lowvol5%: Sharpe={r_asym['Sharpe']:.3f}, MaxDD={r_asym['MaxDD']*100:.2f}%")
    ...
    print(f"  周收益差均值(2%-3%)={obs_mean*100:.4f}%/周, P(差<=0)={p_le_zero:.3f}, n={len(diff)}周")
